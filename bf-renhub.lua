...
repeat task.wait()
    if game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild("Main"):FindFirstChild("ChooseTeam") then
        if getgenv().Setting["Join Team"] == "Pirate" then
            for i,v in pairs(getconnections(game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild("Main"):FindFirstChild("ChooseTeam").Container.Pirates.Frame.ViewportFrame.TextButton.Activated)) do
                v.Function()
            end
        elseif getgenv().Setting["Join Team"] == "Marine" then
            for i,v in pairs(getconnections(game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild("Main"):FindFirstChild("ChooseTeam").Container.Marines.Frame.ViewportFrame.TextButton.Activated)) do
                v.Function()
            end
        else
            for i,v in pairs(getconnections(game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild("Main"):FindFirstChild("ChooseTeam").Container.Pirates.Frame.ViewportFrame.TextButton.Activated)) do
...
function CheckQuest()
	...
	if ThreeWorld then
        ...
        elseif MyLevel >= 2300 and MyLevel <= 2324 then 
			...
			CFrameQuest = CFrame.new(231.75, 37.9003029, -12200.292)
			CFrameMon = CFrame.new(359.787811, 173.968323, -12120.8105)
        ...
        elseif MyLevel >= 2350 and MyLevel <= 2374 then 
			...
			CFrameMon = CFrame.new(423.782562, 24.8288345, -12809.585)
...
local function GetNearestPlayersToMouse()
    local MMouse = game:GetService("Players").LocalPlayer:GetMouse()
    local MaxDist, Closest = 1000
    for i,v in pairs(game:GetService("Players"):GetPlayers()) do
        if v.Character:WaitForChild("HumanoidRootPart") then
            ...
            local Dist = (Vector2.new(Pos.X, Pos.Y) - Vector2.new(MMouse.X, MMouse.Y)).magnitude
            if Dist < MaxDist and Dist < 250 then
                MaxDist = Dist
                _G.CharacterAimBot = v
            end
        end
    end
    return _G.CharacterAimBot
end
...
task.spawn(function()
    game:GetService("RunService").Stepped:Connect(function()
        if farm or AutoFarmChest or FramBossSelectHop or AutoNew or Auto_Farm or AutoNew or Factory or Autothird or MasteryDevilFruit or MasteryWeapon or MasteryGun or FramBoss or KillAllBoss or AutoMobAura or Observation or AutoSaber or AutoPole or AutoPoleHOP or AutoQuestBartilo or AutoEvoRace2 or AutoRengoku or AutoFramEctoplasm or AutoBuddySwords or AutoBuddySwords or AutoFarmBone or AutoHallowScythe or AutoSoulReaper or AutoFarmCakePrince or AutoYama or HolyTorch or AutoEliteHunter or AutoHakiRainbow or MusketeeHat or AutoObservationHakiV2 or NextIsland or AutoRaids or TweenIsland or TweenNPC then
    ...
    old = hookmetamethod(game, "__namecall", newcclosure(function(...)
        ...
        if tostring(method) == "FireServer" and tostring(args[1]) == "RemoteEvent" then
            if tostring(args[2]) ~= "true" and tostring(args[2]) ~= "false" then
                ...
                elseif SkillPlayeraimbot then
                    local HitPart = GetNearestPlayersToMouse()
                    if HitPart then
                        args[2] = AimbotSkill
                        return old(unpack(args))
                    end
        ...
        if tostring(method) == "InvokeServer" then
            --[[if Aimbot and tostring(args[1]) == "RemoteFunctionShoot" then
                args[2] = AimbotGun
                return old(unpack(args))
            else]]
            if Aimbot and tostring(args[1]) == "" then
                local HitPart = GetNearestPlayersToMouse()
                if HitPart then
                    args[3] = AimbotGun
                    return old(unpack(args))
...
function GetCurrentBlade()
    local ret = CombatFrameworkR.activeController.blades[1]
...
task.spawn(function()
    RigLib.wrapAttackAnimationAsync = function(p11, p12, p13, p14, p15)
        local bladehit2 = require(game:GetService("ReplicatedStorage").CombatFramework.RigLib).getBladeHits(
            game:GetService("Players").LocalPlayer.Character, {game:GetService("Players").LocalPlayer.Character.HumanoidRootPart}, 60
        )
        local cac2 = {}
        local hash2 = {}
        for k, v in pairs(bladehit2) do
            if v.Parent:FindFirstChild("HumanoidRootPart") and not hash2[v.Parent] then
                table.insert(cac2, v.Parent.HumanoidRootPart)
                hash2[v.Parent] = true
            end
        end
        if #cac2 > 0 then
...
task.spawn(function()
    game:GetService("RunService").Stepped:Connect(function()
        pcall(function()
            ...
            game:GetService("Players").LocalPlayer.Characters.Stun = 0
            if fastattect then
...
local function FastAttackFunction(fastwait)
    local u8 = debug.getupvalue(CombatFrameworkR.activeController.attack, 5)
    local u9 = debug.getupvalue(CombatFrameworkR.activeController.attack, 6)
    local u7 = debug.getupvalue(CombatFrameworkR.activeController.attack, 4)
    local u10 = debug.getupvalue(CombatFrameworkR.activeController.attack, 7)
    ...
    if fastattect then
        for i = 1, 1 do 
            ...
            if #cac > 0 then
                debug.setupvalue(CombatFrameworkR.activeController.attack, 5, u8)
                debug.setupvalue(CombatFrameworkR.activeController.attack, 6, u9)
                debug.setupvalue(CombatFrameworkR.activeController.attack, 4, u7)
                debug.setupvalue(CombatFrameworkR.activeController.attack, 7, u10)
                if game:GetService("Players").LocalPlayer.Character:FindFirstChildOfClass("Tool") and CombatFrameworkR.activeController.blades and CombatFrameworkR.activeController.blades[1] then 
                    for k, v in next, CombatFrameworkR.activeController.animator.anims.basic do
                        v:Play(0,0.0001,0)
                    end
                    ...
                    game:GetService("ReplicatedStorage").RigControllerEvent:FireServer("hit", cac, i, "")
                end
            end
        end
        task.wait(fastwait)
                ...
                if farm then
                    if FarmMode == "AutoFarmLevel" then
                        if game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible == false then
                            ...
                            Questtween = toTarget(NPCQuestCFrame.Position,NPCQuestCFrame) task.wait(.1)
...
AutoFarmTab:Toggle("Super Fast Attack (with No Attack Animation)", false,function(value)
                    ...
                    if MyLevel >= 700 and OldWorld and AutoNew then
                        ...
                        if game.ReplicatedStorage.Remotes.CommF_:InvokeServer("DressrosaQuestProgress", "Dressrosa") ~= 0 then
                            if Workspace.Map.Ice.Door.Transparency == 1 then
                                ...
                                elseif game.Workspace.Enemies:FindFirstChild("Ice Admiral [Lv. 700] [Boss]") and game.Workspace.Map.Ice.Door.CanCollide == false and game.Workspace.Map.Ice.Door.Transparency == 1 and (CFrame.new(1347.7124, 37.3751602, -1325.6488).Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).magnitude <= 300 then
                                    if DoorNewWorldTween then DoorNewWorldTween:Stop() end
                                    CheckBoss = true
            ...
            repeat task.wait()
                if Factory then
                    ...
                    elseif game.ReplicatedStorage:FindFirstChild("Core") and game.ReplicatedStorage:FindFirstChild("Core"):FindFirstChild("Humanoid") then
                        ...
                        if (CFrame.new(448.46756, 199.356781, -441.389252).Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).magnitude <= 300 then
                            if Farmtween then
                                GOtween:Stop()
                            end
    ...
    task.spawn(function()
        repeat task.wait(.25)
            if Superhuman and game.Players.LocalPlayer:FindFirstChild("WeaponAssetCache") then 
                ...
                if game.Players.LocalPlayer.Backpack:FindFirstChild("Electro") and game.Players.LocalPlayer.Backpack:FindFirstChild("Electro").Level.Value >= 100 or game.Players.LocalPlayer.Character:FindFirstChild("Electro") and game.Players.LocalPlayer.Character:FindFirstChild("Electro").Level.Value >= 300 then
                    local args = {
                        [1] = "BuyBlackLeg"
                    }
                    game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer(unpack(args))
...
AutoFarmTab:Toggle("Auto Superhuman [Full]", getgenv().Setting["Auto Superhuman [Full]"],function(vu)
    ...
    task.spawn(function()
        repeat task.wait(.25)
            if SuperhumanFull and game.Players.LocalPlayer:FindFirstChild("WeaponAssetCache") then 
                if game.Players.LocalPlayer.Backpack:FindFirstChild("Combat") or game.Players.LocalPlayer.Character:FindFirstChild("Combat") then
                    local args = {
                        [1] = "BuyElectro"
                    }
...
function AutoFramAllBoss()
    for i, v in pairs(game.ReplicatedStorage:GetChildren()) do
        if not KillBossuseGet then
            ...
            elseif v.Name == "Magma Admiral [Lv. 350] [Boss]" and game.Players.localPlayer.Data.Level.Value >= 325  then
    ...
    while true do game:GetService("RunService").RenderStepped:Wait()
        if game.Players.LocalPlayer.Character:FindFirstChild("Humanoid") and game.Players.LocalPlayer.Character.Humanoid.Health > 0 then
            if MagnetMobAura and AutoMobAura then
                for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                    if MagnetMobAura and AutoMobAura and v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and v.Humanoid.Health > 0 and (v.HumanoidRootPart.Position - game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position).magnitude <= 300 then
                        v.HumanoidRootPart.CFrame = PosMon
...
AutoFarmMiscTab:Toggle("Auto Farm Observation",getgenv().Setting["Auto Farm Observation"],function(a)
    Observation = a
            ...
            if Observation then
                if ThreeWorld then
                    if game.Workspace.Enemies:FindFirstChild("Musketeer Pirate [Lv. 1925]") then
                        if game.Players.LocalPlayer.PlayerGui.ScreenGui:FindFirstChild("ImageLabel") then
                            repeat task.wait()
                                if (game.Workspace.Enemies:FindFirstChild("Musketeer Pirate [Lv. 1925]").HumanoidRootPart.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).magnitude > 300 then
                                ...
                                elseif (game.Workspace.Enemies:FindFirstChild("Musketeer Pirate [Lv. 1925]").HumanoidRootPart.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).magnitude <= 300 then
                                    if Farmtween then
                                        Farmtween:Stop()
                                    end
                        ...
                        if game.Workspace.Enemies:FindFirstChild("Ship Engineer [Lv. 1275]") and (CFrame.new(-13330, 449, -9919).Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).magnitude <= 300 then
                            if Modstween then
                                Modstween:Stop()
                            end
                ...
                elseif NewWorld then
                    if game.Workspace.Enemies:FindFirstChild("Ship Engineer [Lv. 1275]") then
                        if game.Players.LocalPlayer.PlayerGui.ScreenGui:FindFirstChild("ImageLabel") then
                            repeat task.wait()
                                if (game.Workspace.Enemies:FindFirstChild("Ship Engineer [Lv. 1275]").HumanoidRootPart.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).magnitude > 300 then
                                ...
                                elseif (game.Workspace.Enemies:FindFirstChild("Ship Engineer [Lv. 1275]").HumanoidRootPart.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).magnitude <= 300 then
                                    if Farmtween then
                                        Farmtween:Stop()
                                    end
                        ...
                        if game.Workspace.Enemies:FindFirstChild("Ship Engineer [Lv. 1275]") and (CFrame.new(918.558777, 40.0827065, 32766.498).Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).magnitude <= 300 then
                            if Modstween then
                                Modstween:Stop()
                            end
                ...
                elseif OldWorld then
                    if game.Workspace.Enemies:FindFirstChild("Shanda [Lv. 475]") then
                        if game.Players.LocalPlayer.PlayerGui.ScreenGui:FindFirstChild("ImageLabel") then
                            repeat task.wait()
                                if (game.Workspace.Enemies:FindFirstChild("Shanda [Lv. 475]").HumanoidRootPart.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).magnitude > 300 then
                                    Farmtween = toTarget(game.Workspace.Enemies:FindFirstChild("Shanda [Lv. 475]").HumanoidRootPart.Position,game.Workspace.Enemies:FindFirstChild("Shanda [Lv. 475]").HumanoidRootPart.CFrame)
                                elseif (game.Workspace.Enemies:FindFirstChild("Shanda [Lv. 475]").HumanoidRootPart.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).magnitude <= 300 then
                                    if Farmtween then
                                        Farmtween:Stop()
                                    end
                                    game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = game.Workspace.Enemies:FindFirstChild("Shanda [Lv. 475]").HumanoidRootPart.CFrame * CFrame.new(3,0,0)
                                end
                            until Observation == false or not game.Players.LocalPlayer.PlayerGui.ScreenGui:FindFirstChild("ImageLabel")
                        else
                            queue_on_teleport([[getgenv().Setting["Auto Farm Observation"] = true; loadstring(readfile("edit-renhub.lua")))()]])
                            game:GetService("TeleportService"):TeleportToPlaceInstance(game.PlaceId, game.JobId)
                            repeat task.wait()
                                game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = game.Workspace.Enemies:FindFirstChild("Shanda [Lv. 475]").HumanoidRootPart.CFrame * CFrame.new(10,15,0)
                            until Observation == false or game.Players.LocalPlayer.PlayerGui.ScreenGui:FindFirstChild("ImageLabel")
                        end
                    else
                        Modstween = toTarget(CFrame.new(5533.29785, 88.1079102, 4852.3916).Position, CFrame.new(5533.29785, 88.1079102, 4852.3916))
                        if game.Workspace.Enemies:FindFirstChild("Ship Engineer [Lv. 1275]") and (CFrame.new(5533.29785, 88.1079102, 4852.3916).Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).magnitude <= 300 then
                            if Modstween then
                                Modstween:Stop()
                            end
...
AutoFarmMiscTab:Toggle("Auto Buy Midnight Blade",_G.AutoBuyMidnightBlade,function(A)
    if NewWorld then
        ...
        task.spawn(function()
            ...
            until not AutoBuyMidnightBlade
        end)
    else
        Flux:Notification("Use in New World")
    end
...
function autoTushita()
    if _G.checkup and not game.Players.LocalPlayer.Backpack:FindFirstChild("God's Chalice") and not game.Players.LocalPlayer.Character:FindFirstChild("God's Chalice") then
        ...
        elseif HopFunction then
            MytoTarget(CFrame.new(-12554.9443, 337.194092, -7501.44727, 0.906377554, 4.23948272e-08, -0.422468632, -1.89128269e-08, 1, 5.97740595e-08, 0.422468632, -4.61877896e-08, 0.906377554))				task.wait(1)
            HopServer()
        end
    ...
    if ThreeWorld then
        if game.ReplicatedStorage.Remotes.CommF_:InvokeServer("CitizenQuestProgress", "Citizen") == 4 or game.ReplicatedStorage.Remotes.CommF_:InvokeServer("CitizenQuestProgress", "Citizen") == 3 and game:GetService("Players").LocalPlayer.VisionRadius == 3000 then
            ...
            task.spawn(function()
                                ...
                                if ( game.Players.LocalPlayer.Character:FindFirstChild("Pineapple") or game.Players.LocalPlayer.Backpack:FindFirstChild("Pineapple") ) and ( game.Players.LocalPlayer.Character:FindFirstChild("Banana") or game.Players.LocalPlayer.Backpack:FindFirstChild("Banana") ) and ( game.Players.LocalPlayer.Character:FindFirstChild("Apple") or game.Players.LocalPlayer.Backpack:FindFirstChild("Apple") ) then
                                    ...
                                    if game:GetService("Workspace").AppleSpawner:FindFirstChild("Apple") and not ( game.Players.LocalPlayer.Character:FindFirstChild("Apple") or game.Players.LocalPlayer.Backpack:FindFirstChild("Apple") ) then  
                                        for i,v in pairs(game:GetService("Workspace").AppleSpawner:GetChildren()) do
                                            if v:IsA("Tool") then task.wait(1)
                ...
                until not AutoObservationHakiV2
            end)
...
elseif ThreeWorld then
    Island = {
        ...
        ["Hydra Island House"] = CFrame.new(5245, 602, 251),
        ...
        ["Chocolate Island"] = CFrame.new(154, 12, -11980),
        ["Cursed Dual Katana"] = CFrame.new(-12517, 602, -6549)
    }
    -- NPC
    NPC = {
    ...
    task.spawn(function()
        repeat task.wait()
            if TweenIsland then
                ...
                if (Island[SelectedIsland].Position - game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position).magnitude <= 300 then
                    if TweenIslandWork then
                        TweenIslandWork:Stop()
                    end
                    game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame = Island[SelectedIsland]
                end
            end
        until not TweenIsland
    end)
...
TeleportTab:Toggle("Teleport NPC",false,function(a)
    ...
    task.spawn(function()
        repeat task.wait()
            if TweenNPC then
                TweenNPCWork = tweenTarget(NPC[SelectedNPC].Position,NPC[SelectedNPC])
                if (NPC[SelectedNPC].Position - game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position).magnitude <= 300 then
                    if TweenNPCWork then
                        TweenNPCWork:Stop()
                    end
                    game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame = NPC[SelectedNPC]
                end
            end
        until not TweenNPC
    end)
...
PlayerEspTab:Toggle("Aimbot Gun",false,function(bool)
    ...
    task.spawn(function()
        repeat task.wait()
            local PlayersMouse = GetNearestPlayersToMouse()
            pcall(function()
                local PredictedAimbot = PlayersMouse.Character.HumanoidRootPart.CFrame + (PlayersMouse.Character.HumanoidRootPart.Velocity * .165)
                AimbotGun = PredictedAimbot.Position
            end)
        until not Aimbot
    end)
end)
PlayerEspTab:Toggle("Aimbot Skill",false,function(bool)
    SkillPlayeraimbot = bool
    task.spawn(function()
        repeat task.wait()
            local PlayersMouse = GetNearestPlayersToMouse()
            pcall(function()
                local PredictedAimbotSkill = PlayersMouse.Character.HumanoidRootPart.CFrame + (PlayersMouse.Character.HumanoidRootPart.Velocity * .165)
                AimbotSkill = PredictedAimbotSkill.Position
            end)
        until not SkillPlayeraimbot
    end)
end)
-- ESP
...
PlayerEspTab:Toggle("ESP Player",ESPPlayer,function(a)
    ESPPlayer = a
    task.spawn(function()
        repeat task.wait(.1)
...
PlayerEspTab:Toggle("ESP Chest",ChestESP,function(a)
    ChestESP = a
    task.spawn(function()
        repeat task.wait(.1)
...
PlayerEspTab:Toggle("ESP Devil Fruit",DevilFruitESP,function(a)
    DevilFruitESP = a
    task.spawn(function()
        repeat task.wait(.1)
            for i,v in pairs(game:GetService("Workspace"):GetChildren()) do
        ...
        until not DevilFruitESP
    end)
end)
PlayerEspTab:Toggle("ESP Flower",FlowerESP,function(a)
    FlowerESP = a
    task.spawn(function()
        repeat task.wait(.1)
            for i,v in pairs(game:GetService("Workspace"):GetChildren()) do
                    ...
                    if v.Name == "Flower2" or v.Name == "Flower1" then
                        if FlowerESP then 
                            if not v:FindFirstChild('FlowerESP'..Number) then
                                ...
                                v['FlowerESP'..Number].TextLabel.Text = (v.Name ..'   \n'.. round((game:GetService('Players').LocalPlayer.Character.Head.Position - v.Position).Magnitude/3) ..' M')
                            end
                        else
                            if v:FindFirstChild('FlowerESP'..Number) then
                            v:FindFirstChild('FlowerESP'..Number):Destroy()
                            end
                        end   
                    end
                end)
...
PlayerEspTab:Toggle("ESP Real Fruit",RealFruitESP,function(a)
    RealFruitESP = a
    task.spawn(function()
        repeat task.wait(.1)
            for i,v in pairs(game:GetService("Workspace").AppleSpawner:GetChildren()) do
                if v:IsA("Tool") then
                    if RealFruitESP then 
                        if not v.Handle:FindFirstChild('RealFruitESP'..Number) then
                            ...
                            v.Handle['RealFruitESP'..Number].TextLabel.Text = (v.Name ..' '.. round((game:GetService('Players').LocalPlayer.Character.Head.Position - v.Handle.Position).Magnitude/3) ..' M')
                        end
                    else
                        if v.Handle:FindFirstChild('RealFruitESP'..Number) then
                            v.Handle:FindFirstChild('RealFruitESP'..Number):Destroy()
                        end
                    end 
                end
            end
...
MiscTab:Toggle("Unlock Observations Haki V2 (Spoof)",false,function(value)
    obshakiv2 = value
    task.spawn(function()
        repeat task.wait(.2)
            if not game:GetService("CollectionService"):HasTag(game:GetService("Players").LocalPlayer.Character, "KenUpgrade") then
                game:GetService("CollectionService"):AddTag(game:GetService("Players").LocalPlayer.Character, "KenUpgrade")
            end
        until not obshakiv2
    end)
end)
                ...
                if NextIsland then
                    if game.Players.LocalPlayer.PlayerGui.Main.Timer.Visible ~= false then
                        if game:GetService("Workspace")["_WorldOrigin"].Locations:FindFirstChild("Island 5") then
                            ...
                            if (game:GetService("Workspace")["_WorldOrigin"].Locations:FindFirstChild("Island 5").Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).magnitude > 300 then
                                IslandTween = toTarget(game:GetService("Workspace")["_WorldOrigin"].Locations:FindFirstChild("Island 5").Position,game:GetService("Workspace")["_WorldOrigin"].Locations:FindFirstChild("Island 5").CFrame)
                            elseif (game:GetService("Workspace")["_WorldOrigin"].Locations:FindFirstChild("Island 5").Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).magnitude <= 300 then
                                if IslandTween then
                                    IslandTween:Stop()
                                end
                        ...
                        elseif game:GetService("Workspace")["_WorldOrigin"].Locations:FindFirstChild("Island 4") then
                            ...
                            if (game:GetService("Workspace")["_WorldOrigin"].Locations:FindFirstChild("Island 4").Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).magnitude > 300 then
                                IslandTween = toTarget(game:GetService("Workspace")["_WorldOrigin"].Locations:FindFirstChild("Island 4").Position,game:GetService("Workspace")["_WorldOrigin"].Locations:FindFirstChild("Island 4").CFrame)
                            elseif (game:GetService("Workspace")["_WorldOrigin"].Locations:FindFirstChild("Island 4").Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).magnitude <= 300 then
                                if IslandTween then
                                    IslandTween:Stop()
                                end
                        ...
                        elseif game:GetService("Workspace")["_WorldOrigin"].Locations:FindFirstChild("Island 3") then
                            ...
                            if (game:GetService("Workspace")["_WorldOrigin"].Locations:FindFirstChild("Island 3").Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).magnitude > 300 then
                                IslandTween = toTarget(game:GetService("Workspace")["_WorldOrigin"].Locations:FindFirstChild("Island 3").Position,game:GetService("Workspace")["_WorldOrigin"].Locations:FindFirstChild("Island 3").CFrame)
                            elseif (game:GetService("Workspace")["_WorldOrigin"].Locations:FindFirstChild("Island 3").Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).magnitude <= 300 then
                                if IslandTween then
                                    IslandTween:Stop()
                                end
                        ...
                        elseif game:GetService("Workspace")["_WorldOrigin"].Locations:FindFirstChild("Island 2") then
                            ...
                            if (game:GetService("Workspace")["_WorldOrigin"].Locations:FindFirstChild("Island 2").Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).magnitude > 300 then
                                IslandTween = toTarget(game:GetService("Workspace")["_WorldOrigin"].Locations:FindFirstChild("Island 2").Position,game:GetService("Workspace")["_WorldOrigin"].Locations:FindFirstChild("Island 2").CFrame)
                            elseif (game:GetService("Workspace")["_WorldOrigin"].Locations:FindFirstChild("Island 2").Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).magnitude <= 300 then
                                if IslandTween then
                                    IslandTween:Stop()
                                end
                        ...
                        elseif game:GetService("Workspace")["_WorldOrigin"].Locations:FindFirstChild("Island 1") then
                            ...
                            if (game:GetService("Workspace")["_WorldOrigin"].Locations:FindFirstChild("Island 1").Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).magnitude > 300 then
                                IslandTween = toTarget(game:GetService("Workspace")["_WorldOrigin"].Locations:FindFirstChild("Island 1").Position,game:GetService("Workspace")["_WorldOrigin"].Locations:FindFirstChild("Island 1").CFrame)
                            elseif (game:GetService("Workspace")["_WorldOrigin"].Locations:FindFirstChild("Island 1").Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).magnitude <= 300 then
                                if IslandTween then
                                    IslandTween:Stop()
                                end
...
Devil_Fruit_Sniper_Tab:Toggle("Bring Devil Fruit",bringdevilfruit,function(value)
    ...
    task.spawn(function()
        repeat task.wait()
            if TeleportDF then
                for i,v in pairs(game:GetService("Workspace"):GetChildren()) do
                    if v:IsA("Tool") and string.find(v.Name,"Fruit") then task.wait(5)
                        firetouchinterest(game.Players.LocalPlayer.Character.HumanoidRootPart,v.Handle,0)    
                    end
                end
            end  
        until not TeleportDF
    end)
end)
...
task.spawn(function()
    while task.wait() do
        if AutoStoreFruits then task.wait()
            for i,v in pairs(game.Players.LocalPlayer.Backpack:GetChildren()) do
                if string.find(v.Name,"Fruit") then
                    local FruitName = RemoveSpaces(v.Name)
                    if v.Name == "Bird: Falcon Fruit" then
