...
AutoFarmTab:Toggle("Super Fast Attack (with Auto Clicker)", false,function(value)
...
spawn(function()
    while true do wait()
        ...
        if TweenNPC then
            ...
            if (NPC[SelectedNPC].Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).magnitude <= 350 then
                if TweenNPCWork then
                    TweenNPCWork:Stop()
                end
                game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = NPC[SelectedNPC]
            end
        end
    end
end)
...
PlayerEspTab:Toggle("Bounty Hop",getgenv().Setting["Bounty Hop"],function(bool)
    BountyHop = bool
end)
...
spawn(function()
	game:GetService('RunService').Stepped:connect(function()
		if BountyHop == true or _G.HopServerOn == true then
			NoclipPart.Position = Vector3.new(game.Players.LocalPlayer.Character.HumanoidRootPart.Position.x,game.Players.LocalPlayer.Character.HumanoidRootPart.Position.y - 3.95,game.Players.LocalPlayer.Character.HumanoidRootPart.Position.z)
		end
	end)
end)
...
if _G.DistancePlayers == nil then
	_G.DistancePlayers = 60
end

if _G.TimeToNextPlayer == nil then
	_G.TimeToNextPlayer = 6
end
							...
							if _G.GodModeOn then
								if game.Players.LocalPlayer.Backpack:FindFirstChild(_G.WeaponUseKillGun) or game.Players.LocalPlayer.Character:FindFirstChild(_G.WeaponUseKillGun) then
									...
									if not game.Players.LocalPlayer.Backpack:FindFirstChild(_G.WeaponUseKillGun) and not game.Players.LocalPlayer.Character:FindFirstChild(_G.WeaponUseKillGun) then
										if NewWorld then
											MytoTarget(CFrame.new(124.16289520264, 21.941701889038, 2848.5454101563))
										end
										if ThreeWorld then
											MytoTarget(CFrame.new(-216.48155212402, 9.3963174819946, 5322.3149414063))
										end
										wait(.3)
										local args = {
											[1] = "StoreItem",
...
spawn(function()
	game:GetService('RunService').Stepped:connect(function()
		pcall(function()
			if BountyHop and game.Players.LocalPlayer.Character:FindFirstChild(_G.WeaponUseKillGun).RemoteFunctionShoot then
				local args = {
...
_G.CooldownKill = false
function CooldownKill()
	spawn(function()
		_G.CooldownKill = true
		wait(3)
		_G.CooldownKill = false
	end)
end
...
spawn(function()
	while wait(_G.TimeToNextPlayer) do
		pcall(function()
			if BountyHop then
				for i, v in pairs(game:GetService("Workspace").Characters:GetChildren()) do
					if v.Name ~= NameKill and v.Humanoid.Health ~= v.Humanoid.MaxHealth and KillHealt == MaxKillHealt and _G.CooldownKill == false then
						NextPlayer = true
						CooldownKill()
					end
				end
...
spawn(function()
	while wait(8) do
		pcall(function()
			if BountyHop then
				for i, v in pairs(game:GetService("Workspace").Characters:GetChildren()) do
					if v.Name == NameKill and v.Humanoid.Health == v.Humanoid.MaxHealth and _G.CooldownKill == false then
						NextPlayer = true
						CooldownKill()
					end
				end
    ...
    game:GetService('Players').PlayerAdded:Connect(function(Player)
        Player.CharacterAdded:Connect(function(v)
            pcall(function()
                if not isnil(v) then
                    if ESPPlayer then
                        if not isnil(v.Head) and not v.Head:FindFirstChild('PlayerESP'..Number) then
                            ...
                            v.Head['PlayerESP'..Number].TextLabel.Text = (v.Name ..' | '.. round((game:GetService('Players').LocalPlayer.Character.Head.Position - v.Head.Position).Magnitude/3) ..' M\nHealth : ' .. round(v.Humanoid.Health*100/v.Humanoid.MaxHealth) .. '%')
                        end
                    else
                        if v.Head:FindFirstChild('PlayerESP'..Number) then
                            v.Head:FindFirstChild('PlayerESP'..Number):Destroy()
                        end
                    end
                end
            end)
...
PlayerEspTab:Toggle("ESP Devil Fruit",DevilFruitESP,function(a)
    ...
    game:GetService("Workspace").ChildAdded:Connect(function(v)
        pcall(function()
            if DevilFruitESP then
                if string.find(ok.Name, "Fruit") then   
                    if not ok.Handle:FindFirstChild('FruitESP'..Number) then
                        local bill = Instance.new('BillboardGui',ok.Handle)
                        bill.Name = 'FruitESP'..Number
                        bill.ExtentsOffset = Vector3.new(0, 1, 0)
                        bill.Size = UDim2.new(1,200,1,30)
                        bill.Adornee = v.Handle
...
PlayerEspTab:Toggle("ESP Real Fruit",RealFruitESP,function(a)
    ...
    game:GetService("Workspace").AppleSpawner.ChildAdded:Connect(function(v)
        if v:IsA("Tool") then
            if RealFruitESP then 
                if not v.Handle:FindFirstChild('RealFruitESP'..Number) then
                    local bill = Instance.new('BillboardGui',v.Handle)
    ...
    game:GetService("Workspace").PineappleSpawner.ChildAdded:Connect(function(v)
        if v:IsA("Tool") then
            if RealFruitESP then 
                if not v.Handle:FindFirstChild('RealFruitESP'..Number) then
                    local bill = Instance.new('BillboardGui',v.Handle)
    ...
    game:GetService("Workspace").BananaSpawner.ChildAdded:Connect(function(v)
        if v:IsA("Tool") then
            if RealFruitESP then 
                if not v.Handle:FindFirstChild('RealFruitESP'..Number) then
                    local bill = Instance.new('BillboardGui',v.Handle)
